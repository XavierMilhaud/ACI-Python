...
class TestWindComponent(unittest.TestCase):
    ...
    def test_wind_thresholds(self):
        ...
        wind = WindComponent(
            ...
            self.v10_path,
            self.mask_path)
        wind_thresholds = wind.wind_thresholds(
        ...
        # Check that the result contains the correct time period
        self.assertGreaterEqual(
            wind_thresholds['time'].min(),
            np.datetime64('2000-01-01'))
        self.assertLessEqual(
            wind_thresholds['time'].max(),
            np.datetime64('2020-12-31'))
    ...
    def test_days_above_thresholds(self):
        ...
        wind = WindComponent(
            ...
            self.v10_path,
            self.mask_path)
        days_above_thresholds = wind.days_above_thresholds(
            self.reference_period)
        ...
        # Check that the result contains the correct time period
        self.assertGreaterEqual(
            days_above_thresholds['time'].min(),
            np.datetime64('2000-01-01'))
        self.assertLessEqual(
            days_above_thresholds['time'].max(),
            np.datetime64('2020-12-31'))
    ...
    def test_wind_exceedance_frequency(self):
        ...
        wind = WindComponent(
            ...
            self.v10_path,
            self.mask_path)
        wind_exceedance_frequency = wind.wind_exceedance_frequency(
            self.reference_period)

        # Verify that wind_exceedance_frequency is a DataArray
        self.assertIsInstance(
            wind_exceedance_frequency,
            xr.DataArray)
        ...
        # Check that the result contains the correct time period
        self.assertGreaterEqual(
            wind_exceedance_frequency['time'].min(),
            np.datetime64('2000-01-01'))
        self.assertLessEqual(
            wind_exceedance_frequency['time'].max(),
            np.datetime64('2020-12-31'))
    ...
    def test_std_wind_exceedance_frequency(self):
        ...
        wind = WindComponent(
            ...
            self.v10_path,
            self.mask_path)
        standardized_frequency = wind.std_wind_exceedance_frequency(
            self.reference_period)
        ...
        # Check that the result contains the correct time period
        self.assertGreaterEqual(
            standardized_frequency['time'].min(),
            np.datetime64('2000-01-01'))
        self.assertLessEqual(
        ...
        self.assertFalse(
            ...
            "Std standardized frequency should not be NaN.")
        ...
        for test_case in test_cases:
            with self.subTest(test_case=test_case):
                ...
                reference_anomalies_path = os.path.join(
                    self.data_dir,
                    f'{test_case}_reference_anomalies.nc')

                reference_anomalies = xr.open_dataset(reference_anomalies_path)
                w_com = WindComponent(u10_path, v10_path, mask_path)

                calculated_anomalies = w_com.std_wind_exceedance_frequency(
                    self.reference_period_bis,
                    area=True)

                reference_variable_name = list(
                    reference_anomalies.data_vars)[0]
                np.testing.assert_allclose(
                    calculated_anomalies.values,
                    reference_anomalies[reference_variable_name].values)
